package com.yuaihen.wcdxg.base

/**
 * Created by Yuaihen.
 * on 2021/5/27
 * App内接口API
 */
object NetConstants {

    const val APP_BASE_URL = "https://www.wanandroid.com/"
    const val REGISTER = "user/register"
    const val LOGIN = "user/login"
    const val LOGOUT = "user/logout/json"
    const val GET_BANNER = "banner/json"
    const val GET_TOP_ARTICLE = "article/top/json"
    const val COLLECT_OTHER_WEBSITE_ARTICLE = "lg/collect/add/json"
    const val GET_COLLECT_WEBSITE = "lg/collect/usertools/json"
    const val COLLECT_WEBSITE = "lg/collect/addtool/json"
    const val UPDATE_COLLECT_WEBSITE = "lg/collect/updatetool/json"
    const val DELETE_COLLECT_WEBSITE = "lg/collect/deletetool/json"



}